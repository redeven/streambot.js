...
export const STREAMER_LIST_FACTORY: CommandFactory = (configService) => {
  return {
    data: new SlashCommandBuilder().setName('streamerlist').setDescription('Show list of streamers'),
    execute: (interaction: CommandInteraction) => {
      if (interaction.guild?.id) {
        const SOURCES = Object.entries(configService.getConfiguration().guilds[interaction.guild.id].sources).map(
          ([name, source]) =>
